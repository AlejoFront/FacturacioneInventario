...
$query = "SELECT orders.id_order, orders.client_id, orders.fecha_add, orders.total_neto, orders.descuento, orders.total, orders.metodo, 
			  orders.fecha_liqui, orders.user_add_id, clients.id_client, clients.nombre_cte
	          FROM orders 
	          INNER JOIN clients 
	          ON orders.client_id = clients.id_client 
	          WHERE orders.client_id = $client  AND orders.tipo_orden = 2 ";
$resultado = $connect->query($query);

$query2 = "SELECT id_client, nombre_cte FROM clients WHERE id_client = $client ";
...
$pdf->Cell(100,6,'',0,0);
$pdf->Cell(80,6, 'Cliente:	'.$row2['nombre_cte'],0,1);
$pdf->Ln(2);
