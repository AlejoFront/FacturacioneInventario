...
$desde = date('d/m/Y', strtotime($_POST['fcha1C']));
$hasta = date('d/m/Y', strtotime($_POST['fcha2D']));

include '../php_action/db_connect.php';
require_once  'fpdf/fpdf.php';

$query = "SELECT orders.id_order, orders.client_id, orders.fecha_add, orders.total_neto, 
                 orders.descuento, orders.total, orders.metodo, orders.fecha_liqui, 
                 orders.user_add_id, user.id_user, user.nombre, clients.id_client, clients.nombre_cte
	          FROM orders 
	          INNER JOIN user 
