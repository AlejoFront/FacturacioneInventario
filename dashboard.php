...
//Total de Cleintes
$sqlA = "SELECT * FROM clients WHERE status = 1";
$queryA = $connect->query($sqlA);
$countProductA = $queryA->num_rows;
//Total de Clintes Oro
$sqlB = "SELECT * FROM clients WHERE status = 1 AND categoria = 1";
$queryB = $connect->query($sqlB);
$countProductB = $queryB->num_rows;
//Total de Clintes Plata
$sqlC = "SELECT * FROM clients WHERE status = 1 AND categoria = 2";
$queryC = $connect->query($sqlC);
$countProductC = $queryC->num_rows;
//Consignaciones a vencer
$query = "SELECT orders.id_order, orders.fecha_add, orders.estado, orders.client_id, 
                 orders.saldo, orders.fecha_liqui, orders.tipo_orden, clients.id_client, clients.nombre_cte 
          FROM orders 
          INNER JOIN clients 
...
//Ventas a vencer
$queryV ="SELECT orders.id_order, orders.fecha_add, orders.estado, orders.client_id, orders.saldo, 
                 orders.fecha_liqui, orders.tipo_orden, clients.id_client, clients.nombre_cte 
          FROM orders 
          INNER JOIN clients 
          ON orders.client_id = clients.id_client
