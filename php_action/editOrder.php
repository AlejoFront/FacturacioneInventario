...
if($_POST) {
	...
	for($x = 0; $x < count($_POST['codProd']); $x++) {
		//  product table
		$updateProductCantidadSql = "SELECT products.cantidad FROM products WHERE products.id_product = ".$_POST['codProd'][$x].".";
		...
		while ($updateProductCantidadResult = $updateProductCantidadData->fetch_row()) {
			...
			$updateCantidadSql = "UPDATE products SET cantidad = $editCantidad WHERE id_product = ".$_POST['codProd'][$x].".";
	...
	if($updateDetalleOrden) {
		...
		for($x = 0; $x < count($_POST['codProd']); $x++) {			
			...
			while ($updateProductCantidadResult = $updateProductCantidadData->fetch_row()) {
					...
					$updateProductTable = "UPDATE products SET cantidad = '".$updateCantidad[$x]."' WHERE id_product = ".$_POST['codProd'][$x].".";
