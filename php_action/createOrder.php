...
if ($_POST) {
	...
	for($x = 0; $x < count($_POST['codProd']); $x++) {			
		$updateProductQuantitySql = "SELECT products.cantidad FROM products WHERE products.id_product = ".$_POST['codProd'][$x]."";
		...
		while ($updateProductQuantityResult = $updateProductQuantityData->fetch_row()) {
			$updateQuantity[$x] = $updateProductQuantityResult[0] - $_POST['quantity'][$x];							
				...
				$updateProductTable = "UPDATE products SET cantidad = '".$updateQuantity[$x]."' WHERE id_product = ".$_POST['codProd'][$x]."";
